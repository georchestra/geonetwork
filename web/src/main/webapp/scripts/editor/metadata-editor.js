...
function disableEditForm()
{
	...
	$('editFormTable') && $('editFormTable').insert({'top':editorOverlay});
	$('editorOverlay') && $('editorOverlay').setStyle({opacity: "0.65"});
}
					...
					if (name !== null) {
						...
						if (Modalbox.options.urlbtn !== null) {
							...
							if ((gmdUrlField !== undefined) && (Modalbox.options.mdid !== undefined)) {
								gmdUrlField.value = Env.host + getGNServiceURL('resources.get') + "?name=" + name.value + "&id=" + Modalbox.options.mdid;
							}
						}
