...
public class DataManager
{
  ...
  public void updateDisplayOrder(Dbms dbms, String id, String displayOrder) throws Exception {
    ...
    */
      ...
      String schema = getMetadataSchema(dbms, id);
        ...
        if (!xmlInputs.isEmpty()) {
            ...
            for (String ref : xmlInputs.keySet()) {
              String value = xmlInputs.get(ref);
                ...
                else if ((value.startsWith("<gmd:MD_Keywords xmlns:gmd=\"http://www.isotc211.org/2005/gmd\">")) && (schema.equals("iso19139.fra")))
                {
                  Element fragment = Xml.loadString(value, false);
                    ...
                    XPath alreadyPresent = XPath.newInstance("gmd:identificationInfo/gmd:MD_DataIdentification/gmd:topicCategory/gmd:MD_TopicCategoryCode");
                    ...
                    for (String currentKeyword : laddedKw)
                    {
                      ...
                      /* else */
                        ...
                        {
                          isoThemesInject = String.format(isoThemeSkel, isoThemeToAdd);
                        }
                        else
                        {
                          isoThemesInject += String.format(isoThemeSkel, isoThemeToAdd);
        ...
        if (isoThemesInject != null)
        {
          ...
          XPath xpath = XPath.newInstance("gmd:identificationInfo/gmd:MD_DataIdentification");  
            ...
            // second case : no ISO themes, but a (or maybe more) gmd:characterSet
            else if (curCharset.size() > 0)
            {                
              postoinsert = pos.indexOf(curCharset.get(curCharset.size() - 1)) + 1;
            }
          // third case : no ISO themes, nor Charset
          // inserting right after the <gmd:language> which actually seems mandatory
            ...
            else
            {
              ...
              postoinsert = -1;
                Log.error(Geonet.DATA_MANAGER, "Unable to find a gmd:language element, which should be mandatory.");
            }
          
            if ((pos != null) && (postoinsert != -1))
            {
              Element newEl = Xml.loadString(String.format("<root xmlns:gmd=\"http://www.isotc211.org/2005/gmd\">%s</root>",isoThemesInject), false);
     ...
     *  GeoBretagne CU-09
     *  
     *  @brief Returns the string related to the Iso Theme associated with
     *  the given INSPIRE keyword passed as argument. (@see topicCategoryMapping.xml)
     *  
     *  @param String the INSPIRE keyword
