...
public class DataManager {
    ...
    private static final String isoThemeSkel = "  <gmd:topicCategory>\n"
        + "    <gmd:MD_TopicCategoryCode>%s</gmd:MD_TopicCategoryCode>\n"
        + "  </gmd:topicCategory>\n";
    ...
    public DataManager(ServiceContext context, SearchManager sm,
            AccessManager am, Dbms dbms, SettingManager ss, String baseURL,
            String htmlCacheDir, String dataDir, String appPath)
    throws Exception {
        searchMan = sm;
        accessMan = am;
        settingMan = ss;
     ...
     * 
     **/
    public synchronized void init(ServiceContext context, Dbms dbms,
            Boolean force) throws Exception {

        if (rebuilding)
            throw new OperationNotAllowedEx(
            "Index rebuilding already in progress");
        ...
        for (String id : docs.keySet()) {
            ...
            Log.debug(Geonet.DATA_MANAGER, "- removed record (" + id
                    + ") from index");
        }
    }
    ...
    public synchronized void rebuildIndexXLinkedMetadata(ServiceContext context)
    throws Exception {
        ...
        Log.debug(Geonet.DATA_MANAGER, "Will index " + toIndex.size()
                + " records with XLinks");
        if (toIndex.size() > 0) {
        ...
        public void run() {
            ...
            try {
                ...
                while (!context.isServletInitialized()) {
                    Log.debug(Geonet.DATA_MANAGER,
                    "Waiting for servlet to finish initializing..");
    ...
    public static void indexMetadata(Dbms dbms, String id, SearchManager sm)
    throws Exception {
    ...
    public static void indexMetadata(Dbms dbms, String id, SearchManager sm,
            boolean indexGroup) throws Exception {
        try {
            indexMetadataI(dbms, id, sm, indexGroup);
        } catch (Exception e) {
            Log.error(Geonet.DATA_MANAGER,
                    "The metadata document index with id=" + id
    ...
    private static void indexMetadataI(Dbms dbms, String id, SearchManager sm,
            boolean indexGroup) throws Exception {
        ...
        if (XmlSerializer.resolveXLinks()) {
            ...
            if (xlinks.size() > 0) {
                ...
                moreFields.add(makeField("_xlink", sb.toString(), true, true,
                        false));
        ...
        String query = "SELECT schemaId, createDate, changeDate, source, isTemplate, title, uuid, "
            + "isHarvested, owner, groupOwner, popularity, rating FROM Metadata WHERE id = "
            + id;
        ...
        Log
        .debug(Geonet.DATA_MANAGER, "record createDate (" + createDate
                + ")"); // DEBUG
        ...
        moreFields
        .add(makeField("_isHarvested", isHarvested, true, true, false));
        ...
        if (groupOwner != null)
            moreFields.add(makeField("_groupOwner", groupOwner, true, true,
                    false));

        // get privileges
        List operations = dbms.select(
                "SELECT groupId, operationId FROM OperationAllowed "
                + "WHERE metadataId = " + id
                + " ORDER BY operationId ASC").getChildren();
        ...
        for (Object operation1 : operations) {
            ...
            moreFields.add(makeField("_op" + operationId, groupId, true, true,
                    false));
        }
        // get categories
        List categories = dbms.select(
                "SELECT id, name FROM MetadataCateg, Categories "
                + "WHERE metadataId = " + id
                + " AND categoryId = id ORDER BY id").getChildren();

        for (Object category1 : categories) {
            Element category = (Element) category1;
            String categoryName = category.getChildText("name");

            moreFields.add(makeField("_cat", categoryName.toLowerCase(), true,
                    true, false));
        }
    ...
    public void addSchema(String id, String xmlSchemaFile,
            String xmlSuggestFile, String xmlSubstitutesFile) throws Exception {
        editLib
        .addSchema(id, xmlSchemaFile, xmlSuggestFile,
                xmlSubstitutesFile);
    }
    ...
    public String getMetadataSchema(Dbms dbms, String id) throws Exception {
        List list = dbms.select(
                "SELECT schemaId FROM Metadata WHERE id = " + id).getChildren();

        if (list.size() == 0)
            throw new IllegalArgumentException("Metadata not found for id : "
                    + id);
        else {
            // get metadata
    ...
    // --------------------------------------------------------------------------
    /**
     * Create XML schematron report for each set of rules defined in schema
     * directory.
     */
    private Element getSchemaTronXmlReport(String schema, Element md,
        ...
        for (String rule : rules) {
            ...
            report.setAttribute("rule",
                    rule.substring(0, rule.indexOf(".xsl")), Edit.NAMESPACE);

            String schemaTronXmlXslt = metadataSchema.getSchemaDir()
            + File.separator + rule;
            try {
                Map<String, String> params = new HashMap<String, String>();
                params.put("lang", lang);
                params.put("rule", rule);
                Element xmlReport = Xml
                .transform(md, schemaTronXmlXslt, params);
                if (xmlReport != null)
    ...
    throws Exception {
        ...
        try {
            xsdErrors = Xml.validateInfo(schemaDir + Geonet.File.SCHEMA, md,
                    errorHandler);
        } catch (Exception e) {
    ...
    public String extractUUID(String schema, Element md) throws Exception {
        String styleSheet = editLib.getSchemaDir(schema)
        + Geonet.File.EXTRACT_UUID;
    ...
    public List<Element> getMetadataByHarvestingSource(Dbms dbms,
            String harvestingSource) throws Exception {
    ...
    public String getMetadataId(ServiceContext srvContext, String uuid)
    throws Exception {
    ...
    public void setTemplate(Dbms dbms, int id, String isTemplate, String title)
    throws Exception {
    ...
    public void setTemplateExt(Dbms dbms, int id, String isTemplate,
            String title) throws Exception {
        if (title == null)
            dbms.execute("UPDATE Metadata SET isTemplate=? WHERE id=?",
                    isTemplate, id);
        else
            dbms.execute(
                    "UPDATE Metadata SET isTemplate=?, title=? WHERE id=?",
                    isTemplate, title, id);
    }

    // --------------------------------------------------------------------------

    public void setHarvested(Dbms dbms, int id, String harvestUuid)
    throws Exception {
    ...
    public void setHarvestedExt(Dbms dbms, int id, String harvestUuid)
    throws Exception {
        String value = (harvestUuid != null) ? "y" : "n";
        if (harvestUuid == null) {
            dbms.execute("UPDATE Metadata SET isHarvested=? WHERE id=?", value,
                    id);
        } else {
            dbms
            .execute(
                    "UPDATE Metadata SET isHarvested=?, harvestUuid=? WHERE id=?",
                    value, harvestUuid, id);
        }
    }
    ...
    public void setHarvestedExt(Dbms dbms, int id, String harvestUuid,
            String harvestUri) throws Exception {
        String value = (harvestUuid != null) ? "y" : "n";
    ...
    public String autodetectSchema(Element md) {
        ...
        Log.debug(Geonet.DATA_MANAGER,
                ...
                + metadataAdditionalNS.toString());

        if (md.getName().equals("Record")
                && md.getNamespace().equals(Csw.NAMESPACE_CSW)) {
            return "csw-record";
            ...
            /*
             * there are some other suggested container names, like <dc>,
             * <dublinCore>, <resource>, <record> and <metadata> We may need to
             * also check for those on import and export
             */
            ...
            // this is important for profiles which usually need to override the
            // top level
            // element for proper definition
            // eg. mcp:MD_Metadata versus wmo:MD_Metadata
            //
            // But profil for france does not override top level element. But
            // only sub
            // elements.
            // 
            // we suppose that the root element declare the prime namespace of
            // the profil
            ...
            // xmlns:fra="http://www.cnig.gouv.fr/2005/fra">
            // if profil specific namespace only declared on sub elements and
            // not on root.
            ...
            for (String schema : getSchemas()) {
                ...
                for (Namespace ns : metadataAdditionalNS) {
                    if (ns.equals(Csw.NAMESPACE_CSW)
                            || ns.equals(Csw.NAMESPACE_GFC))
                        continue;

                    if (ns.getURI().equals(primeNs)
                            && metadatadRootElemenNSUri
                            .equals(Csw.NAMESPACE_GMD)) {
                        return schema;
                    }
                }
            }
    ...
    // --------------------------------------------------------------------------
    public void updateDisplayOrder(Dbms dbms, String id, String displayOrder)
    throws Exception {
    ...
    public int rateMetadata(Dbms dbms, int id, String ipAddress, int rating)
    throws Exception {
        ...
        Log.debug(Geonet.DATA_MANAGER, "Setting rating for id:" + id
                + " --> rating is:" + rating);
    ...
    public String createMetadata(Dbms dbms, String templateId,
            String groupOwner, SerialFactory sf, String source, int owner,
        ...
        Element xml = updateFixedInfoNew(schema, Integer.toString(serial), Xml
                .loadString(data, false), uuid, parentUuid);

        // --- store metadata

        String id = XmlSerializer.insert(dbms, schema, xml, serial, source,
                uuid, owner, groupOwner);

        copyDefaultPrivForGroup(dbms, id, groupOwner);

        // --- store metadata categories copying them from the template

        List categList = dbms.select(
                "SELECT categoryId FROM MetadataCateg WHERE metadataId = "
                + templateId).getChildren();
    ...
    /**
     * @param source
     *            the source of the metadata. If null, the local siteId will be
     *            used
     */

    public String insertMetadataExt(Dbms dbms, String schema, Element md,
            int id, String source, String createDate, String changeDate,
            String uuid, int owner, String groupOwner, String isTemplate)
    throws Exception {
        if (source == null)
            source = getSiteID();
        ...
        // --- Note: we cannot index metadata here. Indexing is done in the
        // harvesting part

        String record = XmlSerializer.insert(dbms, schema, md, id, source,
                uuid, createDate, changeDate, isTemplate, null, owner,
                groupOwner);
    ...
    // --------------------------------------------------------------------------
    /**
     * Adds a metadata in xml form (the xml should be validated). The group id
     * is used to setup permissions. Internal metadata fields are updated.
     * Default operations are set.
     */

    public String insertMetadata(Dbms dbms, String schema, String category,
            String groupId, Element xml, SerialFactory sf, String source,
            String uuid, int owner) throws Exception {
    ...
    public String insertMetadata(Dbms dbms, String schema, String category,
            String groupOwner, Element xml, SerialFactory sf, String source,
            String uuid, String isTemplate, String title, int owner)
    throws Exception {
        // --- generate a new metadata id
        int serial = sf.getSerial(dbms, "Metadata");
        ...
        if (isTemplate.equals("n"))
            xml = updateFixedInfoExisting(schema, Integer.toString(serial),
                    xml, uuid);

        // --- store metadata

        String id = XmlSerializer.insert(dbms, schema, xml, serial, source,
                uuid, isTemplate, title, owner, groupOwner);
     ...
     * requested and does NOT include validation errors
     */
    ...
    throws Exception {
        ...
        if (forEditing) { // copy in xlink'd fragments but leave xlink atts to
            ...
            if (withEditorValidationErrors) {
                // -- get an XSD validation report and add results to the
                // metadata
            ...
            if (withEditorValidationErrors) {
                // -- get a schematron error report if no xsd errors and add
                // results
                // -- to the metadata as a geonet:schematronerrors element with
                // -- links to the ref id of the affected element
                Element condChecks = getSchemaTronXmlReport(schema, md,
    ...
    public boolean existsMetadata(Dbms dbms, int id) throws Exception {
        ...
        List list = dbms.select("SELECT id FROM Metadata WHERE id=" + id)
        .getChildren();
        return list.size() != 0;
    }
    ...
    public boolean existsMetadataUuid(Dbms dbms, String uuid) throws Exception {
        ...
        List list = dbms.select(
                "SELECT uuid FROM Metadata WHERE uuid='" + uuid + "'")
                .getChildren();
        return list.size() != 0;
    }
    ...
    private Element getMetadataFromSession(UserSession session, String id) {
        Log.debug(Geonet.DATA_MANAGER, "Retrieving metadata from session "
                + session.getUserId());
        Element md = (Element) session
        .getProperty(Geonet.Session.METADATA_EDITING + id);
        md.detach();
        return md;
    }
    ...
    private void setMetadataIntoSession(UserSession session, Element md,
            String id) {
        Log.debug(Geonet.DATA_MANAGER, "Storing metadata in session "
                + session.getUserId());
    ...
    public void removeMetadataEmbedded(UserSession session, String id) {
        Log.debug(Geonet.DATA_MANAGER, "Removing metadata from session "
                + session.getUserId());
     ...
     */
    public Element getMetadataEmbedded(ServiceContext srvContext, String id,
            boolean forEditing, boolean withValidationErrors) throws Exception {
        Element md = getMetadata(srvContext, id, forEditing,
     ...
     */
    public synchronized Element addElementEmbedded(Dbms dbms,
            UserSession session, String id, String ref, String name,
        ...
        // --- later re-use
        Element refEl = (Element) (el.getChild(Edit.RootChild.ELEMENT,
                Edit.NAMESPACE)).clone();
        Element info = (Element) (md.getChild(Edit.RootChild.INFO,
                Edit.NAMESPACE)).clone();
        md.removeChild(Edit.RootChild.INFO, Edit.NAMESPACE);
    ...
    public synchronized Element deleteElementEmbedded(Dbms dbms,
            UserSession session, String id, String ref, String parentRef)
    throws Exception {
        ...
        if (parent != null) {
            ...
            // --- check and see whether the element to be deleted is the last
            // one
            Filter elFilter = new ElementFilter(uName, ns);
            if (parent.getContent(elFilter).size() == 1) {

                // --- get geonet child element with attribute name =
                // unqualified name
                Filter chFilter = new ElementFilter(Edit.RootChild.CHILD,
                        Edit.NAMESPACE);
            ...
            throw new IllegalStateException("Element at ref = " + ref
    ...
    public synchronized void swapElementEmbedded(Dbms dbms,
            UserSession session, String id, String ref, boolean down)
    throws Exception {
        ...
        List list = ((Element) elSwap.getParent()).getChildren(
                elSwap.getName(), elSwap.getNamespace());
        ...
        if (iSwapIndex == -1)
            throw new IllegalStateException("Index not found for element --> "
                    + elSwap);
    ...
    public synchronized boolean updateMetadataEmbedded(UserSession session,
        ...
        for (Enumeration e = changes.keys(); e.hasMoreElements();) {
            ...
            Element el = editLib.findElement(md, ref);
            if (el == null)
                Log.error(Geonet.DATA_MANAGER, "Element not found at ref = "
                        + ref);
        ...
        if (isoThemesInject != null) {
            ...
            Namespace gmdNs = Namespace.getNamespace("gmd",
            ...
            charsetPath.addNamespace(gmdFra);
            ...
            languagePath.addNamespace(gmdFra);
            ...
            // mandatory
                ...
                postoinsert = pos.indexOf(curLang.get(curLang.size() - 1)) + 1;
            } else {
                // did not find any way of inserting our data.
                // giving up so that we won't mess up the MD
                postoinsert = -1;
                Log
                .error(Geonet.DATA_MANAGER,
                        "Unable to find a gmd:language element, which should be mandatory.");
            }

            if ((pos != null) && (postoinsert != -1)) {
                Element newEl = Xml
                .loadString(
                        String
                        .format(
                                "<root xmlns:gmd=\"http://www.isotc211.org/2005/gmd\">%s</root>",
                                isoThemesInject), false);
     ...
     * GeoBretagne CU-09
     * 
     * @brief Returns the string related to the Iso Theme associated with the
     *        given INSPIRE keyword passed as argument. (@see
     *        topicCategoryMapping.xml)
     * 
     * @param String
     *            the INSPIRE keyword
     * @return String the associated ISO theme to be injected into the MD
     * 
     * @author pmauduit
     */
    private String getIsoTheme(String val) throws JDOMException {
        ...
        for (int i = 0; i < kws.size(); i++) {
            ret = new String(kws.get(i).getAttributeValue("value"));
        }
        return ret;
    }
     ...
     *            metadata record
     * @param ref
     *            current ref of element. All _lang_AB_123 element will be
     *            processed.
     * @param val
     * @return
     */
    private boolean updatedLocalizedTextElement(Element md, String ref,
            String val) {
        if (ref.startsWith("lang")) {
            if (val.length() > 0) {
                ...
                // --- add new translation
                Namespace gmd = Namespace.getNamespace("gmd",
                "http://www.isotc211.org/2005/gmd");
                ...
                Element refElem = new Element(Edit.RootChild.ELEMENT,
                        Edit.NAMESPACE);
    ...
    /**
     * If no PT_FreeText element exist create a geonet:element with an empty
     * ref.
     * 
     * @param parent
     * @param name
    ...
    public Element validateMetadataEmbedded(UserSession session, Dbms dbms,
    ...
    // --------------------------------------------------------------------------
    /**
     * For Editing : adds an attribute from a metadata ([add] link) FIXME:
     * Modify and use within Ajax controls
     */
    ...
    // --------------------------------------------------------------------------
    /**
     * For Editing : removes an attribute from a metadata ([del] link) FIXME:
     * Modify and use within Ajax controls
     */

    public synchronized boolean deleteAttribute(Dbms dbms, String id,
            String ref, String name, String currVersion) throws Exception {
        Element md = XmlSerializer.select(dbms, "Metadata", id);
    ...
    public synchronized void updateMetadataOwner(Dbms dbms, String id,
            String owner, String groupOwner) throws Exception {
        updateMetadataOwner(dbms, new Integer(id), owner, groupOwner);
    }

    public synchronized void updateMetadataOwner(Dbms dbms, int id,
            String owner, String groupOwner) throws Exception {
    ...
    public synchronized boolean updateMetadata(UserSession session, Dbms dbms,
            String id, String currVersion, Hashtable changes, boolean validate,
            String lang) throws Exception {
        Element md = XmlSerializer.select(dbms, "Metadata", id);
        ...
        for (Enumeration e = changes.keys(); e.hasMoreElements();) {
            ...
            Element el = editLib.findElement(md, ref);
            if (el == null)
                throw new IllegalStateException("Element not found at ref = "
                        + ref);

            if (attr != null) {
                // The following work-around decodes any attribute name that has
                // a COLON in it
                // The : is replaced by the word COLON in the xslt so that it
                // can be processed
                // by the XML Serializer when an update is submitted - a better
                // solution is
                // to modify the argument handler in Jeeves to store arguments
                // with their name
    ...
    // --------------------------------------------------------------------------
    /**
     * Update a metadata record. Clean current validation report in session. If
     * user ask for validation the validation report will be (re-)created then.
     */
    public synchronized boolean updateMetadata(UserSession session, Dbms dbms,
            String id, Element md, boolean validate, String version, String lang)
    throws Exception {
        session.removeProperty(Geonet.Session.VALIDATION_REPORT);

        // --- check if the metadata has been modified from last time
        if (version != null && !editLib.getVersion(id).equals(version)) {
            Log.error(Geonet.DATA_MANAGER, "Version mismatch: had " + version
    ...
    public boolean validate(Element xml) {
        ...
        // XSD validation error(s)
            ...
            Log.debug(Geonet.DATA_MANAGER, "invalid metadata: "
                    + x.getMessage());
            return false;
        }
        return true;
     ...
     * the session.
     * 
     * FIXME : if a record A is in edit mode, only validation report could be
     * retrieve for record A. If asked for record B, validateMetadataEmbedded
     * will use A (stored in session) instead of B ?
     * 
     * @param session
     * @param schema
     * @param id
     ...
     * @throws Exception
     */
    public Element doValidate(UserSession session, String schema, String id,
            Element md, String lang) throws Exception {
        Log.debug(Geonet.DATA_MANAGER,
                "Creating validation report for record #" + id + ".");

        Element sessionReport = (Element) session
        .getProperty(Geonet.Session.VALIDATION_REPORT);
        ...
        if (schemaTronXml != null && schemaTronXml.getContent().size() > 0) {
            ...
            // throw new
            // SchematronValidationErrorEx("Schematron errors detected - see schemaTron report for "+id+" in htmlCache for more details",schematron);
        }
    ...
    public void updateMetadataExt(Dbms dbms, String id, Element md,
    ...
    public synchronized void deleteMetadata(Dbms dbms, String id)
    throws Exception {
        ...
        dbms.execute("DELETE FROM MetadataRating WHERE metadataId=?",
                new Integer(id));
    ...
    public void deleteMetadataOper(Dbms dbms, String id, boolean skipAllIntranet)
    throws Exception {
    ...
    public Element getThumbnails(Dbms dbms, String id) throws Exception {
        ...
        String styleSheet = editLib.getSchemaDir(schema)
        + Geonet.File.EXTRACT_THUMBNAILS;
    ...
    public void setThumbnail(Dbms dbms, String id, boolean small, String file)
    throws Exception {
    ...
    public void unsetThumbnail(Dbms dbms, String id, boolean small)
    throws Exception {
        Element env = new Element("env");
    ...
    private void manageThumbnail(Dbms dbms, String id, boolean small,
            Element env, String styleSheet) throws Exception {
        Element md = XmlSerializer.select(dbms, "Metadata", id);
    ...
    void transformMd(Dbms dbms, String id, Element md, Element env,
            String schema, String styleSheet) throws Exception {
    ...
    public void setOperation(Dbms dbms, String mdId, String grpId, String opId)
    throws Exception {
        setOperation(dbms, new Integer(mdId), new Integer(grpId), new Integer(
                opId));
    }

    public void setOperation(Dbms dbms, int mdId, int grpId, int opId)
    throws Exception {
        ...
        if (elRes.getChildren().size() == 0) {
            dbms
            .execute(
                    "INSERT INTO OperationAllowed(metadataId, groupId, operationId) VALUES(?,?,?)",
                    mdId, grpId, opId);
        }
    }
    ...
    public void unsetOperation(Dbms dbms, String mdId, String grpId, String opId)
    throws Exception {
        unsetOperation(dbms, new Integer(mdId), new Integer(grpId),
                new Integer(opId));
    }

    public void unsetOperation(Dbms dbms, int mdId, int groupId, int operId)
    throws Exception {
    ...
    public void copyDefaultPrivForGroup(Dbms dbms, String id, String groupId)
    throws Exception {
    ...
    public void setCategory(Dbms dbms, String mdId, String categId)
    throws Exception {
        ...
        if (!isCategorySet(dbms, mdId, categId))
            dbms
            .execute(
                    "INSERT INTO MetadataCateg(metadataId, categoryId) VALUES(?,?)",
                    args);
    }

    // --------------------------------------------------------------------------

    public boolean isCategorySet(Dbms dbms, String mdId, String categId)
    throws Exception {
        String query = "SELECT metadataId FROM MetadataCateg "
            + "WHERE metadataId=? AND categoryId=?";

        Element elRes = dbms.select(query, new Integer(mdId), new Integer(
                categId));
    ...
    public void unsetCategory(Dbms dbms, String mdId, String categId)
    throws Exception {
    ...
    private void swapElements(Element el1, Element el2) throws Exception {
        ...
        if (index1 == -1) {
            ...
            "Element 1 not found for swapping");
        }
        int index2 = parent.indexOf(el2);
        if (index2 == -1) {
            throw new IllegalArgumentException(
            "Element 2 not found for swapping");
        }
    ...
    private Element updateFixedInfo(String schema, String id, Element md,
            Dbms dbms) throws Exception {
        String query = "SELECT uuid, source, isTemplate FROM Metadata WHERE id = "
            + id;
    ...
    public Element updateFixedInfoExisting(String schema, String id,
            Element md, String uuid) throws Exception {
        ...
        env.addContent(new Element("datadir").setText(Lib.resource.getDir(
                dataDir, Params.Access.PRIVATE, id)));
        return updateFixedInfo(schema, md, env);
    }

    // --------------------------------------------------------------------------

    public Element updateFixedInfoNew(String schema, String id, Element md,
            String uuid, String parentUuid) throws Exception {
        // --- setup environment - for new records
     ...
     * @throws Exception
     */
    public Map<String, Element> getUnnotifiedMetadata(Dbms dbms,
            String notifierId) throws Exception {
        ...
        Log.debug(Geonet.DATA_MANAGER, "getUnnotifiedMetadata after select: "
                + (results != null));
        ...
        if (results != null) {
            for (Element result : results) {
                ...
                unregisteredMetadata.put(uuid, (Element) ((Element) result
                        .clone()).detach());

            }
        }

        Log.debug(Geonet.DATA_MANAGER, "getUnnotifiedMetadata returning #"
                + unregisteredMetadata.size() + " results");
     ...
     * @throws Exception
     */
    public Map<String, Element> getUnnotifiedMetadataToDelete(Dbms dbms,
            String notifierId) throws Exception {
        Log.debug(Geonet.DATA_MANAGER, "getUnnotifiedMetadataToDelete start");
        Map<String, Element> unregisteredMetadata = new HashMap<String, Element>();
        Log.debug(Geonet.DATA_MANAGER,
        "getUnnotifiedMetadataToDelete after dbms");
        ...
        List<Element> results = dbms.select(query).getChildren();
        Log.debug(Geonet.DATA_MANAGER,
                "getUnnotifiedMetadataToDelete after select: "
                + (results != null));
        ...
        Log.debug(Geonet.DATA_MANAGER,
                "getUnnotifiedMetadataToDelete returning #"
                + unregisteredMetadata.size() + " results");
        return unregisteredMetadata;
    }
     ...
     *            Metadata identifier
     * @param notifierId
     *            Notifier service identifier
     * @param deleteNotification
     *            Indicates if the notification was a delete action
     * @param dbms
     * @throws Exception
     */
    public void setMetadataNotified(String metadataId, String metadataUuid,
            String notifierId, boolean deleteNotification, Dbms dbms)
    throws Exception {
        ...
        if (!deleteNotification) {
            ...
            dbms.execute(query, new Integer(metadataId),
                    new Integer(notifierId), metadataUuid, "y", "u");
            dbms.commit();
        }

        Log.debug(Geonet.DATA_MANAGER,
                "setMetadataNotified finished for metadata with id "
                + metadataId + "and notitifer with id " + notifierId);
    }
     ...
     * @throws Exception
     */
    public void setMetadataNotifiedError(String metadataId,
            String metadataUuid, String notifierId, boolean deleteNotification,
            String error, Dbms dbms) throws Exception {
        System.out.println("setMetadataNotifiedError");
        try {
            String query = "DELETE FROM MetadataNotifications WHERE metadataId=? AND notifierId=?";
            dbms.execute(query, new Integer(metadataId),
                    new Integer(notifierId));
            ...
            dbms.execute(query, new Integer(metadataId),
                    new Integer(notifierId), metadataUuid, "n", action, error);
            dbms.commit();

            Log.debug(Geonet.DATA_MANAGER,
                    "setMetadataNotifiedError finished for metadata with id "
                    + metadataId + "and notitifer with id "
                    + notifierId);
        } catch (Exception ex) {
            ex.printStackTrace();
            throw ex;
    ...
    /**
     * Applies automatic fixes to the metadata (if that is enabled) and adds
     * some environmental information about this system.
     * 
     * @param schema
     * @param md
     * @param env
     ...
     * @throws Exception
     */
    private Element updateFixedInfo(String schema, Element md, Element env)
    throws Exception {
        boolean autoFixing = settingMan.getValueAsBool(
                "system/autofixing/enable", true);
        if (autoFixing) {
            Log.debug(Geonet.DATA_MANAGER,
            "Autofixing is enabled, applying update-fixed-info");

            Element result = new Element("root");
            result.addContent(md);

            // --- environment common to both existing and new records goes here
            env.addContent(new Element("changeDate").setText(new ISODate()
            .toString()));
            env.addContent(new Element("siteURL").setText(getSiteURL()));
            Element system = settingMan.get("system", -1);
            env.addContent(Xml.transform(system, appPath
                    + Geonet.Path.STYLESHEETS + "/xml/config.xsl"));
    ...
    private Element buildInfoElem(ServiceContext context, String id,
        ...
        // add operations
        Element operations = accessMan.getAllOperations(context, id, context
                .getIpAddress());
        HashSet<String> hsOper = accessMan.getOperations(context, id, context
                .getIpAddress(), operations);

        addElement(info, Edit.Info.Elem.VIEW, String.valueOf(hsOper
                .contains(AccessManager.OPER_VIEW)));
        addElement(info, Edit.Info.Elem.NOTIFY, String.valueOf(hsOper
                .contains(AccessManager.OPER_NOTIFY)));
        addElement(info, Edit.Info.Elem.DOWNLOAD, String.valueOf(hsOper
                .contains(AccessManager.OPER_DOWNLOAD)));
        addElement(info, Edit.Info.Elem.DYNAMIC, String.valueOf(hsOper
                .contains(AccessManager.OPER_DYNAMIC)));
        addElement(info, Edit.Info.Elem.FEATURED, String.valueOf(hsOper
                .contains(AccessManager.OPER_FEATURED)));

        if (!hsOper.contains(AccessManager.OPER_DOWNLOAD)) {
            boolean gDownload = Xml.selectNodes(
                    operations,
                    "guestoperations/record[operationid="
                    + AccessManager.OPER_DOWNLOAD
                    + " and groupid='-1']").size() == 1;
        ...
        for (Object category1 : categories) {
            Element category = (Element) category1;
            addElement(info, Edit.Info.Elem.CATEGORY, category
                    .getChildText("name"));
        }
        ...
        String host = settingMan.getValue("system/server/host");
        String port = settingMan.getValue("system/server/port");
        addElement(info, Edit.Info.Elem.BASEURL, "http://" + host
                + (port == "80" ? "" : ":" + port) + baseURL);
    ...
    /**
     * Add missing namespace (ie. GML) to XML inputs. It should be done by the
     * client side but add a check in here.
     * 
     * @param fragment
     *            The fragment to be checked and processed.
     * 
    ...
    private MetadataSchema findSchema(Element md, Namespace ns)
    throws Exception {
        ...
        for (String schema : getSchemas()) {
            ...
            if (nsSchema != null && nsUri.equals(nsSchema)) {
                Log.debug(Geonet.DATA_MANAGER, "Found schema " + schema
                        + " with NSURI " + nsSchema);
                return mds;
            }
        }
        ...
        throw new IllegalArgumentException(
                "Cannot find a namespace to set for element "
                + md.getQualifiedName() + " with namespace URI "
                + nsUri);
    }

    // --------------------------------------------------------------------------

    private void notifyMetadataChange(Dbms dbms, Element md, String id)
    throws Exception {
        String isTemplate = getMetadataTemplate(dbms, id);

        if (isTemplate.equals("n")) {
            GeonetContext gc = (GeonetContext) servContext
    ...
    private void notifyMetadataDelete(Dbms dbms, String id, String uuid)
    throws Exception {
        GeonetContext gc = (GeonetContext) servContext
        .getHandlerContext(Geonet.CONTEXT_NAME);
    ...
    /**
     * Update group owner when handling privileges during import. Does not
     * update the index.
     * 
     * @param dbms
     * @param mdId
    ...
    throws Exception {
        ...
        Element capabilitiesInfoRecord = dbms.select(
                "SELECT * FROM CswServerCapabilitiesInfo WHERE langId = ?",
                language);
    ...
    public void saveCswCapabilitiesInfo(Dbms dbms,
            CswCapabilitiesInfo cswCapabilitiesInfo) throws Exception {

        String langId = cswCapabilitiesInfo.getLangId();

        dbms
        .execute(
                "UPDATE CswServerCapabilitiesInfo SET label = ? WHERE langId = ? AND field = ?",
                cswCapabilitiesInfo.getTitle(), langId, "title");
        dbms
        .execute(
                "UPDATE CswServerCapabilitiesInfo SET label = ? WHERE langId = ? AND field = ?",
                cswCapabilitiesInfo.getAbstract(), langId, "abstract");
        dbms
        .execute(
                "UPDATE CswServerCapabilitiesInfo SET label = ? WHERE langId = ? AND field = ?",
                cswCapabilitiesInfo.getFees(), langId, "fees");
        dbms
        .execute(
                "UPDATE CswServerCapabilitiesInfo SET label = ? WHERE langId = ? AND field = ?",
                cswCapabilitiesInfo.getAccessConstraints(), langId,
        "accessConstraints");
    }
