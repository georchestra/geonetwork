...
public class RoleBasedSynchronizationIT extends AbstractAccountsReconcilingServiceIntegrationTest {
    ...
    public @Test void map_user_org_with_georchestra() {
        List<CanonicalGroup> groupsWithTitle = super.defaultGroups.stream()
            .map(g ->
                "PSC".equals(g.getName()) ?
                    CanonicalGroup.builder().init(g).withOrgTitle("Project Steering Committee").build() : g).collect(toList());
        when(canonicalAccountsRepositoryMock.findAllOrganizations()).thenReturn(groupsWithTitle);
        ...
        //Alter default groups and users to have org titles
        List<CanonicalUser> users = super.defaultUsers.stream()
            .map(u ->
                "PSC".equals(u.getOrganization()) ?
                    CanonicalUser.builder().init(u).withOrganization("Project Steering Committee").build() : u).collect(java.util.stream.Collectors.toList());
