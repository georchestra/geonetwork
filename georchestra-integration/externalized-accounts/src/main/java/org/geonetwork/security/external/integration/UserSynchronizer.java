...
class UserSynchronizer {
    ...
    Map<String, String> canonicalOrgsMap = new HashMap<>();
    ...
    public void synchronizeAll(List<CanonicalUser> canonical) {
        ...
        canonicalOrgsMap = canonicalAccounts.findAllOrganizations().stream()
            .filter(g -> g.getName() != null && g.getOrgTitle() != null)
            .collect(Collectors.toMap(CanonicalGroup::getName, CanonicalGroup::getOrgTitle));
    ...
    private void setGeonetworkUserProperties(CanonicalUser canonical, User target, Profile profile) {
        ...
        // replace organization by org title when possible
        target.setOrganisation(canonicalOrgsMap.getOrDefault(canonical.getOrganization(), canonical.getOrganization()));
